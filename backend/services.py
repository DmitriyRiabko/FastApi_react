...
oauth2schema = _security.OAuth2AuthorizationCodeBearer(tokenUrl='/api/token')
...
async def get_current_user(db: _orm.Session,   token:str = _fastapi.Depends(oauth2schema)):
    try:
    ...
    return _schemas.User.from_attributes(user)